...
- **Stable Verification & Baseline**: Record verification status, CI status, and baseline milestones. Do NOT attempt to commit the SHA generated by the current commit inside tracked files (resolve current HEAD at runtime via `git rev-parse HEAD`).
