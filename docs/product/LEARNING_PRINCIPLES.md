# LEARNING_PRINCIPLES.md — Canonical Pedagogical & Cognitive Invariants

> **Scope**: This document constitutes the non-negotiable pedagogical contract governing the **English Learning OS**. Just as `AGENTS.md` governs software engineering and code hygiene, this document governs all instructional design, routine scheduling, card creation, and learner interactions across all AI agents and human contributors.
...
## 2. Active Retrieval Over Passive Recognition (The Testing Invariant)
- **No Illusion of Competence**: Passive rereading, highlighting, listening to unsegmented audio in the background, or re-watching videos creates a meta-cognitive illusion of learning. It is strictly prohibited as a core study activity.
- **Effortful Reconstruction**: Memory traces are stabilized primarily through the cognitive effort of retrieving information from long-term memory into working memory.
- **Mandatory Elicitation**: Flashcards, comprehension checks, and conversational turns must require the learner to generate, speak, or type the target form before revealing the correct model.
...
## 5. Meaningful Pushed Output & Syntactic Processing
- **Output is Mandatory from Day 1**: Speaking and writing must not be deferred until "later levels". Spoken production must begin in Week 1 using scaffolded sentence frames and micro-dialogues.
- **Syntactic Encoding**: Oral and written production forces the brain to move from semantic processing (getting the gist) to syntactic processing (assembling word order, inflections, and agreements).
- **Separate Fluency from Accuracy**: Fluency practice (speed, continuity, communicative flow) and accuracy drills (precise grammar and pronunciation) have contradictory cognitive demands for beginners. They must be practiced in separate micro-sessions, never conflated into a single stressful exercise.
